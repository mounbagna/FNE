...
const Dashboard = ({ user }) => {
  ...
  const [aUser, setAUser] = useState({ name: "", email: "", phone: "", subject: "", message: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!aUser.name || !aUser.email || !aUser.phone || !aUser.subject || !aUser.message) {
      setErrorMsg("All fields are required");
      return;
    }

    try {
      await axios.post(`${API}/emailsend`, aUser);
      setSuccessMsg("Your email has been sent to the admin successfully");
      setAUser({ name: "", email: "", phone: "", subject: "", message: ""});
      setErrorMsg("");
      ...
      <section className="page-section portfolio" id="portfolio">
        <div className="container">
          ...
          <div className="row justify-content-center">
            {[
              { title: "job1", href: "/apply", img: job1 },
              { title: "job2", href: "/apply", img:job2 },
              { title: "job3", href: "/apply", img: job3 },
              { title: "job4", href: "/apply", img: job4 },
              { title: "job5", href: "/apply", img: job5},
              { title: "job6", href: "/apply", img: job6 },
            ].map((item, index) => (
            ...
            <div className="col-lg-8 col-xl-7">
              <form onSubmit={handleSubmit}>
                <div className="form-floating mb-3">
                  <input className="form-control" name="name" type="text" placeholder="Enter your name..." value={aUser.name} onChange={handleChange} required />
                  <label>Full name</label>
                </div>
                <div className="form-floating mb-3">
                  <input className="form-control" name="email" type="email" placeholder="name@example.com" value={aUser.email} onChange={handleChange}  required />
                  <label>Email address</label>
                </div>
                <div className="form-floating mb-3">
                  <input className="form-control" name="phone" type="tel" placeholder="(123) 456-7890" value={aUser.phone} onChange={handleChange}  required />
                  <label>Phone number</label>
                </div>
                <div className="form-floating mb-3">
                  <input className="form-control" name="subject" type="text" placeholder="Enter a subject..." value={aUser.subject} onChange={handleChange}  required />
                  <label>Subject</label>
                </div>
                <div className="form-floating mb-3">
                  <textarea className="form-control" name="message" placeholder="Enter your message here..." style={{ height: "10rem" }} value={aUser.message} onChange={handleChange}  required></textarea>
                  <label>Message</label>
                </div>
